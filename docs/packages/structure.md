Articles such as [[How do I use WEKA's classes in my own code?]] and [[How do I write a new classifier or filter?]] describe how to extend Weka to add your own learning algorithms and so forth. This article describes how such enhancements can be assembled into a *package* that can be accessed via Weka’s [package management](../packages/manager.md) system. Bundling your enhancements in a package makes it easy to share with other Weka users.
